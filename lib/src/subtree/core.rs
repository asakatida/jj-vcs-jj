...
pub enum SubtreeError {
    ...
    #[error("Prefix conflicts with existing file: {}", .0.as_internal_file_string())]
    PrefixConflict(RepoPathBuf),

    /// No content exists under the specified prefix path.
    #[error("No subtree found at prefix: {}", .0.as_internal_file_string())]
...
/// Returns `SubtreeError::Backend` if there's an error reading or writing
/// trees.
...
async fn commit_modifies_prefix(
    ...
    let parent_tree = if commit.parent_ids().is_empty() {
        ...
        let parent = repo
            .store()
            .get_commit(commit.parent_ids().first().unwrap())?;
        parent.tree()
    };
...
pub fn has_subtree_at_prefix(tree: &MergedTree, prefix: &RepoPath) -> Result<bool, SubtreeError> {
    ...
    if let Some(result) = tree.entries_matching(&matcher).next() {

    ...
    pub fn parse(description: &str) -> Self {
        ...
        for trailer in trailers {
            ...
            // Check for mainline commit
                ...
                if let Ok(id) = CommitId::try_from_hex(&trailer.value) {
...
mod tests {
    ...
    fn test_parse_all_fields() {
        let desc = "Message\n\n\
            Subtree-dir: vendor/lib\n\
            Subtree-mainline: abc123abc123abc123abc123abc123abc123abc123ab\n\
            Subtree-split: def456def456def456def456def456def456def456de\n";
        let meta = SubtreeMetadata::parse(desc);
        assert_eq!(
            meta.subtree_dir,
            Some(RepoPathBuf::from_internal_string("vendor/lib").unwrap())
    ...
    fn test_is_empty() {
        ...
        assert!(!SubtreeMetadata::with_dir(
            RepoPathBuf::from_internal_string("foo").unwrap()
        )
        .is_empty());
    }
}

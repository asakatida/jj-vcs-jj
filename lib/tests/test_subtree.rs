...
fn test_move_tree_to_prefix_nested_directory() {
    ...
    let prefix = RepoPath::from_internal_string("external");
    let result = move_tree_to_prefix(store, &tree, prefix).unwrap();

    // Verify all paths are prefixed correctly
    assert!(has_subtree_at_prefix(
        &result,
        RepoPath::from_internal_string("external/src/main.rs")
    )
    .unwrap());
    assert!(has_subtree_at_prefix(
        &result,
        RepoPath::from_internal_string("external/src/lib/util.rs")
    )
    .unwrap());
    assert!(has_subtree_at_prefix(
        &result,
        RepoPath::from_internal_string("external/README.md")
    )
    .unwrap());

    // Verify original paths don't exist
    assert!(!has_subtree_at_prefix(&result, RepoPath::from_internal_string("src/main.rs")).unwrap());
}
...
fn test_move_tree_to_prefix_preserves_file_contents() {
    ...
    let new_path = RepoPath::from_internal_string("vendor/lib/main.rs");
...
fn test_extract_subtree_single_level() {
    ...
    assert!(has_subtree_at_prefix(&result, RepoPath::from_internal_string("file.rs")).unwrap());

    // src/main.rs should NOT be in the result
    assert!(
        !has_subtree_at_prefix(&result, RepoPath::from_internal_string("src/main.rs")).unwrap()
    );
}
...
fn test_extract_subtree_nested() {
    ...
    assert!(has_subtree_at_prefix(&result, RepoPath::from_internal_string("src/lib.rs")).unwrap());
    assert!(has_subtree_at_prefix(&result, RepoPath::from_internal_string("README.md")).unwrap());

    // Verify other/file.txt is NOT in result
    assert!(
        !has_subtree_at_prefix(&result, RepoPath::from_internal_string("other/file.txt")).unwrap()
    );
}
...
fn test_extract_subtree_no_content_at_prefix() {
    ...
    assert!(!has_subtree_at_prefix(&result, RepoPath::from_internal_string("")).unwrap());
}
...
fn test_extract_subtree_preserves_file_contents() {
    ...
    let value = result
        .path_value(RepoPath::from_internal_string("lib.rs"))
        .unwrap();
    assert!(!value.is_absent());
    assert!(!value.is_tree());
}
...
fn test_has_subtree_exists() {
    ...
    let parent_prefix = RepoPath::from_internal_string("vendor");
...
fn test_prefix_conflicts_file_at_prefix() {
    ...
    assert_eq!(
        result.unwrap().as_internal_file_string(),
        "vendor/lib"
    );
}
...
fn test_prefix_conflicts_file_on_ancestor_path() {
    ...
    let prefix = RepoPath::from_internal_string("vendor/lib/subdir");
    let result = prefix_conflicts_with_file(&tree, prefix).unwrap();
...
fn test_roundtrip_move_and_extract() {
    ...
    assert!(
        has_subtree_at_prefix(&extracted_tree, RepoPath::from_internal_string("src/lib.rs"))
            .unwrap()
    );
    assert!(
        has_subtree_at_prefix(&extracted_tree, RepoPath::from_internal_string("README.md"))
            .unwrap()
    );
}

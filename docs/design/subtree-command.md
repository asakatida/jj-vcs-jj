...
2. Walk commit history, filtering by prefix:
...
3. For commits that touched subtree:
...
**Implementation:**
1. Implement basic split logic (no --rejoin)
2. Add commit filtering with `--keep-empty` / `--skip-empty`
3. Implement synthetic commit creation
4. Implement `--branch` option
5. Handle merge commits properly
6. Add metadata linking
7. Add tests
...
3. **Squash mode**: Add with squash (default), update with squash
4. **Multiple subtrees**: Multiple independent subtrees in same repo
5. **Nested subtrees**: Subtree within a subtree
6. **Merge conflicts**: Handling conflicts during subtree merge
7. **Backend compatibility**: Test on both Git and non-Git backends
...
9. ✅ Squash mode is default and works as expected
10. ✅ User can explicitly choose empty commit handling (--keep-empty / --skip-empty)
11. ✅ Comprehensive test coverage (>80% for new code)
12. ✅ Documentation with examples for all commands
...
4. **Migration tool**: Convert Git subtrees to jj subtrees
5. **Conflict strategies**: Provide merge strategy options (ours, theirs, union)
6. **Performance optimization**: Parallel processing, better caching
7. **Partial subtree**: Support for shallow subtrees (depth limit)
8. **Subtree rename**: Handle moving subtree to different prefix
9. **Nested subtree support**: Full support for subtrees within subtrees
10. **Automatic subtree conflict resolution**: Smart conflict resolution for common cases
11. **Integration with Jujutsu's sparse checkout features**: Efficient handling of large subtrees
12. **Subtree-aware diff and log operations**: Better visualization of subtree changes
13. **Support for different merge strategies per subtree**: Configurable merge behavior</content>
